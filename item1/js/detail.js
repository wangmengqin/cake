...
ajax('get','json/cake-data.json','',function(obj){
		...
		if(arr[1] == obj[i].id){
			str+=`<div class="information" data-id = ${obj[i].id}>
					...
					<div class="info-bottom">
						...
						<ul class="right-box">
							<h3><span>店取</span>${obj[i].title}</h3>
							<li>${obj[i].describ}</li>
							<li>
							...
							<li><label>重量：</label><span class="wei">${obj[i].weight}</span></li>
							<li><label>口味：</label><span class="tas">${obj[i].taste}</span></li>
							<li>请选择："重量" "口味"  </li>
	...
	var tas = document.querySelector('.tas');
	var wei = document.querySelector('.wei');
	tas.onmouseover = function(){
